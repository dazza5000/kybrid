...
import com.fivestars.cordovaalternativepattern.BuildConfig
...
// when / if we read less than its size.
    ...
    fun registerConnectedCallback(connectedCallback: ConnectedCallback) {
        BluetoothSerialService.connectedCallback = connectedCallback
    }

    fun registerClosedCallback(closedCallback: ClosedCallback) {
        BluetoothSerialService.closedCallback = closedCallback
    }

    fun registerDataCallback(dataCallback: DataCallback) {
        BluetoothSerialService.dataCallback = dataCallback;
    }
}